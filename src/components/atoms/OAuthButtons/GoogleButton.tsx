...
export const GoogleButton = ({
  ...
  const handleCredential = (credential: string) => {
    ...
    if (!payload?.email) {
      onError?.("Не удалось получить данные пользователя от Google.");
      return;
    }
  ...
  const onClick = async () => {
    ...
    try {
      ...
      onError?.(
        e instanceof Error ? e.message : "Google авторизация не удалась.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <button
      ...
      title={!clientId ? "Google client id не настроен" : "Войти через Google"}
    >
